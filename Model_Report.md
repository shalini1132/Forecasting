...
## 2. Data preparation
- **Source file:** `time_series_dataset.csv`
...
- **Scaling:** Features scaled using `StandardScaler` prior to LSTM input (fit on train set, transform on test set).
- **Train/test split:** Time-based split (no shuffle). Typical split: 80% train, 20% test.

---

## 3. Baseline model (implemented)
A simple baseline using **Exponential Smoothing** (Holt-Winters without seasonality) was implemented in  Baseline metrics:
- Baseline RMSE:3.5983
- Baseline MAE: 2.9583
